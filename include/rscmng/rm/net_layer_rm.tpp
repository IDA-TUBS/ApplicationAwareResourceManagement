    ...
    {      
        ...
        {            
            ...
            case ERROR_INTERFACE:
                RM_logInfo("ERROR_INTERFACE from client: " << dequeued_message.source_id << " Interface id: " << dequeued_message.destination_id << " Linkspeed : " << dequeued_message.mode)
...
void NetLayerRM<T>::initial_start_handler()
{
    ...
    start_timer.registerTimer(rm_id, start_trigger, false, &NetLayerRM<T>::experiment_selection, this);
...
void NetLayerRM<T>::start_network_traffic_synchronous()
{
    ...
    struct timespec target_time_client_start = prepare_timestamp_add(starting_timestamp, client_start_offset);
    struct timespec time_now = {0,0};
    clock_gettime(CLOCK_REALTIME, &time_now);
    RM_logInfo("Timestamp now               : " << time_now.tv_sec << "s " << time_now.tv_nsec << " ns")
    ...
    uint32_t startup_mode = experiment_parameter_struct.startup_mode;
    ...
    while (iterator != service_list.end()) 
    {
        ...
        uint32_t client_start_mode = 0;

        auto network_mode_it = experiment_parameter_struct.startup_mode_map.find(startup_mode);
        if (network_mode_it != experiment_parameter_struct.startup_mode_map.end()) 
        {
            auto client_id_it = network_mode_it->second.find(client_id);
            if (client_id_it != network_mode_it->second.end()) 
            {
                client_start_mode = client_id_it->second;
                RM_logInfo("Client " << client_id << " found mode: " << client_start_mode)
                // = experiment_parameter_struct.startup_mode_map[startup_mode]

                uint32_t service_id_int =  static_cast<uint32_t>(service_id);
                std::chrono::milliseconds service_offset =  std::chrono::milliseconds(rm_client_service_map[service_id_int][client_start_mode].slot_offset);
                //service_offset = service_offset + std::chrono::milliseconds(experiment_parameter_struct.mc_client_start_offset);
                struct timespec target_time_start_offset = prepare_timestamp_add(target_time_client_start, service_offset);
                rm_active_timestamp_last = target_time_start_offset;
            
                udp::endpoint target_address = get_client_address(data_message.source_id);

                RM_logInfo("Client IDs             : " << std::hex << data_message.source_id << std::dec << " with target address: " << target_address.address().to_string() << " " << target_address.port() << " and slot_offset: " << service_offset.count() << " ms")
                ...
                send_synchronous_start(target_time_start_offset, target_address, service_id, client_start_mode);

            }
            else
            {
                RM_logInfo("Client " << client_id << " no start mode found")
            }
        }
    ...
    while (iterator != service_list.end()) 
    {
        ...
        if (network_mode_it != experiment_parameter_struct.startup_mode_map.end()) 
        {
            ...
            if (client_id_it != network_mode_it->second.end()) 
            {
                ...
                control_channel.send_start(target_address, demonstrator::RM_ID, client_start_mode, service_id, &rm_message);
            }
            else
            {
                RM_logInfo("Client " << client_id << " no start mode found")
...
*/
template<class T>
struct timespec NetLayerRM<T>::prepare_timestamp_add(struct timespec time_stamp, std::chrono::milliseconds delay)
{
...
*/
template<class T>
struct timespec NetLayerRM<T>::prepare_timestamp_sub(struct timespec time_stamp, std::chrono::milliseconds delay)
{
    struct timespec target_time = {0,0};

    // Convert delay to nanoseconds (integer count)
    long long delay_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(delay).count();

    // Compute total nanoseconds for easier math
    long long total_ns = static_cast<long long>(time_stamp.tv_sec) * 1000000000LL + time_stamp.tv_nsec;

    // Subtract delay in nanoseconds
    long long result_ns = total_ns - delay_ns;

    if (result_ns < 0)
    {
        result_ns = 0;  // clamp to zero if it ever underflows
    }

    // Convert back to seconds and nanoseconds
    target_time.tv_sec  = result_ns / 1000000000LL;
    target_time.tv_nsec = result_ns % 1000000000LL;
...
*/
template<class T>
void NetLayerRM<T>::experiment_selection()
{
    if (experiment_parameter_struct.experiment_number == 0)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)   
        experiment_mode_change();  
    }
    else if (experiment_parameter_struct.experiment_number == 1)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)
        experiment_mode_change_time_triggered_asynchonous_objects_hw();
    }
    else if (experiment_parameter_struct.experiment_number == 2)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number) 
        experiment_mode_change_time_triggered_synchronous_objects_hw();  
    }
    else if (experiment_parameter_struct.experiment_number == 3)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)
        experiment_mode_change_time_triggered_asynchonous_objects_sw();   
    }
    else if (experiment_parameter_struct.experiment_number == 4)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)   
        experiment_mode_change_time_triggered_synchronous_objects_sw();
    }
    else if (experiment_parameter_struct.experiment_number == 5)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)
        experiment_mode_change_time_triggered_synchronous_objects_sw();   
    }
    else if (experiment_parameter_struct.experiment_number == 6)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)   
        experiment_mode_change_time_triggered_synchronous_objects_sw();
    }
    else if (experiment_parameter_struct.experiment_number == 7)
    {
        RM_logInfo("NetRM start experiments" << experiment_parameter_struct.synchronous_start_mode << " Number: " << experiment_parameter_struct.experiment_number)   
        experiment_mode_change_time_triggered_synchronous_objects_sw();
    }
    ...
    {
        RM_logInfo("Not a valid experiment")
    }      
}
            ...
            while (list_iterator != service_list.end()) 
            {
                ...
                if (network_mode_it != experiment_parameter_struct.reconfiguration_map.end()) 
                {
                    ...
                    if (client_id_it != network_mode_it->second.end()) 
                    {
                        ...
                        RM_logInfo("Found Mode for client: " << std::hex << client_id << std::dec << " " << configuration_mode << " slot offset: " )
                        ...
                        send_asynchronous_reconfigure(target_address, service_id, configuration_mode, iterator);                        
                    }         
                }     
            }
...
*/
template<class T>
void NetLayerRM<T>::experiment_mode_change_time_triggered_asynchonous_objects_hw()
{
    RM_logInfo(" ")
    ...
    for(uint8_t iterator = 0; iterator < experiment_parameter_struct.experiment_iterations; iterator++)
    {
        ...
        for (const auto& mode_order_iterator : experiment_parameter_struct.reconfiguration_order) 
        {
            ...
            struct timespec target_time_stop_offset     = target_mc_begin;
            struct timespec target_time_reconfig_offset = prepare_timestamp_add(target_mc_begin, experiment_parameter_struct.mc_client_reconfig_offset);
            struct timespec target_time_start_offset    = prepare_timestamp_add(target_mc_begin, experiment_parameter_struct.mc_client_start_offset + experiment_parameter_struct.mc_client_reconfig_offset);
            ...
            while (list_iterator != service_list.end()) 
            {
                ...
                if (network_mode_it != experiment_parameter_struct.reconfiguration_map.end()) 
                {
                    ...
                    if (client_id_it != network_mode_it->second.end()) 
                    {
                        ...
                        send_time_triggered_synchronous_reconfigure_type(target_time_stop_offset, target_time_reconfig_offset, target_time_start_offset, target_address, service_id, configuration_mode, RM_CLIENT_SYNC_TIMESTAMP_RECONFIGURE_HW, iterator);
...
*/
template<class T>
void NetLayerRM<T>::experiment_mode_change_time_triggered_asynchonous_objects_sw()
{
    RM_logInfo(" ")
    RM_logInfo("NetRM async_sync experiment start triggered.") 
    struct timespec time_now = {0,0};
    ...
    for(uint8_t iterator = 0; iterator < experiment_parameter_struct.experiment_iterations; iterator++)
    {
        ...
        for (const auto& mode_order_iterator : experiment_parameter_struct.reconfiguration_order) 
        {
            ...
            struct timespec target_time_stop_offset     = {0,0};
            struct timespec target_time_reconfig_offset = prepare_timestamp_add(target_mc_begin, experiment_parameter_struct.mc_client_reconfig_offset);
            struct timespec target_time_start_offset    = {0,0};
            ...
            while (list_iterator != service_list.end()) 
            {
                ...
                if (network_mode_it != experiment_parameter_struct.reconfiguration_map.end()) 
                {
                    ...
                    if (client_id_it != network_mode_it->second.end()) 
                    {
                        ...
                        send_time_triggered_synchronous_reconfigure_type(target_time_stop_offset, target_time_reconfig_offset, target_time_start_offset, target_address, service_id, configuration_mode, RM_CLIENT_SYNC_TIMESTAMP_RECONFIGURE_SW, iterator);
...
*/
template<class T>
void NetLayerRM<T>::experiment_mode_change_time_triggered_synchronous_objects_hw()
{
    RM_logInfo(" ")
    RM_logInfo("NetRM experiment start triggered.") 
    struct timespec time_now = {0,0};
    ...
    for(uint8_t iterator = 0; iterator < experiment_parameter_struct.experiment_iterations; iterator++)
    {
        ...
        for (const auto& mode_order_iterator : experiment_parameter_struct.reconfiguration_order) 
        {
            ...
            uint32_t number_hyperperiod_distribution = experiment_parameter_struct.mc_distribution_phase_duration.count() / hyperperiod_duration.count() + 1; //ms //calculate number hyperperiods for gap to next
            ...
            while (list_iterator != service_list.end()) 
            {
                ...
                if (network_mode_it != experiment_parameter_struct.reconfiguration_map.end()) 
                {
                    ...
                    if (client_id_it != network_mode_it->second.end()) 
                    {
                        ...
                        uint32_t service_id_int =  static_cast<uint32_t>(data_message.service_id);
                        std::chrono::milliseconds service_offset =  std::chrono::milliseconds(rm_client_service_map[service_id_int][configuration_mode].slot_offset) ;
                        
                        struct timespec target_time_start_offset    = prepare_timestamp_add(target_hyperperiod, service_offset);
                        struct timespec target_time_stop_offset     = {0,0};
                        struct timespec target_time_reconfig_offset = prepare_timestamp_sub(target_time_start_offset, experiment_parameter_struct.mc_client_reconfig_offset);
                        ...
                        send_time_triggered_synchronous_reconfigure_type(target_time_stop_offset, target_time_reconfig_offset, target_time_start_offset, target_address, service_id, configuration_mode, RM_CLIENT_SYNC_TIMESTAMP_RECONFIGURE_SYNC_OBJECT_HW, iterator);
...
*/
template<class T>
void NetLayerRM<T>::experiment_mode_change_time_triggered_synchronous_objects_sw()
{
    RM_logInfo(" ")
    RM_logInfo("NetRM experiment start triggered.") 
    struct timespec time_now = {0,0};
    ...
    for(uint8_t iterator = 0; iterator < experiment_parameter_struct.experiment_iterations; iterator++)
    {
        ...
        for (const auto& mode_order_iterator : experiment_parameter_struct.reconfiguration_order) 
        {
            ...
            RM_logInfo("NetRM target hyperperiod for mode change " << target_hyperperiod.tv_sec << " s " << target_hyperperiod.tv_nsec << " ns" )
            ...
            while (list_iterator != service_list.end()) 
            {
                ...
                if (network_mode_it != experiment_parameter_struct.reconfiguration_map.end()) 
                {
                    ...
                    if (client_id_it != network_mode_it->second.end()) 
                    {
                        ...
                        send_time_triggered_synchronous_reconfigure_type(target_time_stop_offset, target_time_reconfig_offset, target_time_start_offset, target_address, service_id, configuration_mode, RM_CLIENT_SYNC_TIMESTAMP_RECONFIGURE_SYNC_OBJECT_SW, iterator);
...
void NetLayerRM<T>::stopping_experiment_synchronous()
{
    ...
    struct timespec target_time_mc_begin = prepare_timestamp_add(starting_timestamp, mc_begin_offset);
    ...
    while (list_iterator != service_list.end()) 
    {
        ...
        struct timespec target_time_stop_offset     = prepare_timestamp_add(target_time_mc_begin, std::chrono::milliseconds(10));
        struct timespec target_time_reconfig_offset = prepare_timestamp_add(target_time_mc_begin, std::chrono::milliseconds(20));
        struct timespec target_time_start_offset    = prepare_timestamp_add(target_time_mc_begin, std::chrono::milliseconds(20));
...
*/
template<class T>
void NetLayerRM<T>::send_synchronous_reconfigure(struct timespec stop_time, struct timespec recofig_time, struct timespec start_time, udp::endpoint target_address, serviceID_t service_id, uint32_t network_mode, uint32_t mc_iterator)
{
    MessageNet_t rm_message;
    ...
    control_channel.send_sync_reconfig(target_address, demonstrator::RM_ID, service_id, mc_iterator, network_mode, &rm_message);
}
...
*/
template<class T>
void NetLayerRM<T>::send_time_triggered_synchronous_reconfigure_type(struct timespec stop_time, struct timespec recofig_time, struct timespec start_time, udp::endpoint target_address, serviceID_t service_id, uint32_t network_mode, MessageTypes message_type, uint32_t mc_iterator)
{
    MessageNet_t rm_message;
    ...
    control_channel.send_sync_reconfig_type(target_address, demonstrator::RM_ID, service_id, mc_iterator, network_mode, message_type, &rm_message);
}
...
*/
template<class T>
void NetLayerRM<T>::send_asynchronous_reconfigure(udp::endpoint target_address, serviceID_t service_id, uint32_t network_mode, uint32_t mc_iterator)
{
    MessageNet_t rm_message;

    control_channel.send_reconfig(target_address, demonstrator::RM_ID, service_id, mc_iterator, network_mode, &rm_message);
}
...
*/
template<class T>
void NetLayerRM<T>::send_synchronous_start(struct timespec target_time, udp::endpoint target_address, serviceID_t service_id, uint32_t network_mode)
{
    MessageNet_t rm_message;
    ...
    control_channel.send_start_sync(target_address, demonstrator::RM_ID, network_mode, service_id, &rm_message);
}
